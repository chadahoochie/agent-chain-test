...
from agents.review_agent import review_agent as mut
...
def test_model_client_query_model_returns_parsed_json_on_success(monkeypatch):
    expected = [{"line": 9, "comment": "extract method"}]
    payload = {
        "choices": [
...
def test_model_client_review_code_returns_error_when_file_cannot_be_read(tmp_path):
    missing_file = tmp_path / "missing.cs"

    result = mut.ModelClient().review_code(str(missing_file), "{}")

    assert "error" in result
    assert result["error"].startswith("Failed to read code:")


def test_model_client_review_code_passes_prompt_to_query_model(tmp_path, monkeypatch):
    ...
    result = mut.ModelClient().review_code(str(code_file), "{}")

    assert result == {"ok": True}
    assert "Review the following C# code and point out flaws" in seen["prompt"]
    assert "class C { }" in seen["prompt"]


def test_review_agent_process_task_returns_agent_result(monkeypatch):
    agent = mut.ReviewAgent()

    def fake_review_code(code_path, context):
        return {"comments": 1}

    monkeypatch.setattr(agent.model, "review_code", fake_review_code)
    ...
    assert response.output == str({"comments": 1})
    assert response.error == ""


def test_review_agent_health_check_returns_ok():
    agent = mut.ReviewAgent()
...
def test_review_agent_get_state_returns_empty_value():
    agent = mut.ReviewAgent()
...
def test_review_agent_serve_starts_and_stops_server(monkeypatch):
    class FakeServer:
        def __init__(self):
            self.started = False
    ...
    assert isinstance(added["servicer"], mut.ReviewAgent)
