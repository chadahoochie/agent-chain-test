...
from agents.optimizer_agent import optimizer_agent as mut
...
def test_model_client_query_model_returns_parsed_json_on_success(monkeypatch):
    expected = [{"line": 5, "change": "Use StringBuilder"}]
    payload = {
        "choices": [
            {
                "message": {
...
def test_model_client_query_model_returns_error_on_invalid_response_shape(monkeypatch):
    def fake_post(*args, **kwargs):
...
def test_model_client_optimize_code_returns_error_when_file_cannot_be_read(tmp_path):
    missing_file = tmp_path / "missing.cs"

    result = mut.ModelClient().optimize_code(str(missing_file), "{}")

    assert "error" in result
    assert result["error"].startswith("Failed to read code:")


def test_model_client_optimize_code_passes_prompt_to_query_model(tmp_path, monkeypatch):
    ...
    result = mut.ModelClient().optimize_code(str(code_file), "{}")

    assert result == {"ok": True}
    assert "Refactor the following C# code" in seen["prompt"]
    assert "class C { }" in seen["prompt"]


def test_optimizer_agent_process_task_returns_agent_result(monkeypatch):
    agent = mut.OptimizerAgent()

    def fake_optimize_code(code_path, context):
        return {"changes": 1}

    monkeypatch.setattr(agent.model, "optimize_code", fake_optimize_code)
    ...
    assert response.output == str({"changes": 1})
    assert response.error == ""


def test_optimizer_agent_health_check_returns_ok():
    agent = mut.OptimizerAgent()
...
def test_optimizer_agent_get_state_returns_empty_value():
    agent = mut.OptimizerAgent()
...
def test_optimizer_agent_serve_starts_and_stops_server(monkeypatch):
    class FakeServer:
        def __init__(self):
            self.started = False
    ...
    assert isinstance(added["servicer"], mut.OptimizerAgent)
