...
from agents.testing_agent import testing_agent as mut
...
def test_model_client_query_model_returns_parsed_json_on_success(monkeypatch):
    expected = [{"name": "test_happy_path", "description": "covers success"}]
    payload = {
        "choices": [
...
def test_model_client_generate_tests_returns_error_when_file_cannot_be_read(tmp_path):
    missing_file = tmp_path / "missing.cs"

    result = mut.ModelClient().generate_tests(str(missing_file), "{}")

    assert "error" in result
    assert result["error"].startswith("Failed to read code:")


def test_model_client_generate_tests_passes_prompt_to_query_model(tmp_path, monkeypatch):
    ...
    result = mut.ModelClient().generate_tests(str(code_file), "{}")

    assert result == {"ok": True}
    assert "Generate efficient, meaningful, and easily maintainable tests" in seen["prompt"]
    assert "class C { }" in seen["prompt"]


def test_test_agent_process_task_returns_agent_result(monkeypatch):
    agent = mut.TestAgent()

    def fake_generate_tests(code_path, context):
        return {"tests": 2}

    monkeypatch.setattr(agent.model, "generate_tests", fake_generate_tests)
    ...
    assert response.output == str({"tests": 2})
    assert response.error == ""


def test_test_agent_health_check_returns_ok():
    agent = mut.TestAgent()
...
def test_test_agent_get_state_returns_empty_value():
    agent = mut.TestAgent()
...
def test_test_agent_serve_starts_and_stops_server(monkeypatch):
    class FakeServer:
        def __init__(self):
            self.started = False
    ...
    assert isinstance(added["servicer"], mut.TestAgent)
